use crate::core::domain::config::raw::ConfigWebserver;
use crate::core::domain::config::webserver_api_key::WebserverApiKey;
use crate::core::domain::config::webserver_api_key::WebserverApiKeyError;
...
use crate::core::domain::config::webserver_hostname::WebserverHostnameError;
...
use crate::core::domain::config::webserver_jwt_key::WebserverJwtKeyError;
...
impl Webserver {
    ...
    pub fn validate_raw_webserver_config(
        &self,
        log_serv: &impl LoggingRepo,
        raw_webserver_conf: &ConfigWebserver,
    ) {
        match &raw_webserver_conf.hostname {
            Some(raw_hostname) => {
        ...
        match &raw_webserver_conf.base_path {
            Some(raw_base_path) => {
        ...
        match &raw_webserver_conf.shutdown_timeout_secs {
            Some(raw_shutdown_timeout_secs) => {
        ...
        match &raw_webserver_conf.api_key_file {
            Some(raw_api_key_file) => {
        ...
        match &raw_webserver_conf.jwt_key_file {
            Some(raw_jwt_key_file) => {
        ...
        match &raw_webserver_conf.jwt_access_token_expiry_secs {
            Some(raw_jwt_access_token_expiry_secs) => {
                if let Err(err) =
                    WebserverJwtAccessTokenExpirySecs::new(raw_jwt_access_token_expiry_secs)
                {
